    		...
    		<div class="menu-div page_container">
    			<div v-for="item in menu_items" :class="item.parent_class_list" style="display: inline-block;padding: 0 20px;">
    				...
    				<div v-else class="all_caps dropdown" :class="{open : item.show_sub_menu}">
    					<a href="#" class="menu_link dropdown-toggle hvr-underline-from-lef" :id="item.name.id" @click="toggleSubMenu(item.id); item.show_sub_menu = !item.show_sub_menu" data-toggle="dropdown" aria-haspopup="true" :aria-expanded="item.show_sub_menu">{{item.name}} </a>
